...
def open1():
    ...
    def Story2(win):  
        def final(tl: Toplevel, food2, profession2, adjective2, phrase, animal2, verb2, place2, celebrity2, somethingToBuy, things2):   
            ...
            tl.geometry(newGeometry='500x550')  
            Label(tl, text='Story:',  wraplength=tl.winfo_width()).place(x=160, y=310)  
            Label(tl, text=text2, wraplength=tl.winfo_width()).place(x=0, y=330)  
        NewScreen11 = Toplevel(win, bg='yellow')  
        NewScreen11.title("About Me")  
        NewScreen11.geometry('500x500')  
        Label(NewScreen11, text='About Me').place(x=100, y=0)  
        Label(NewScreen11, text='A Food or Silly Word:').place(x=0, y=35)  
        Label(NewScreen11, text='Profession/Job:').place(x=0, y=70)  
        Label(NewScreen11, text='Adjective:').place(x=0, y=105)  
        Label(NewScreen11, text='Phrase/Lyrics/Saying:').place(x=0, y=140)  
        Label(NewScreen11, text='Animal:').place(x=0, y=175)  
        Label(NewScreen11, text='Verb:').place(x=0, y=210)  
        Label(NewScreen11, text='Place:').place(x=0, y=245) 
        Label(NewScreen11, text='Celebrity:').place(x=0, y=280) 
        Label(NewScreen11, text='Something you would buy:').place(x=0, y=315) 
        Label(NewScreen11, text='Things (plural):').place(x=0, y=345) 
        food2 = Entry(NewScreen11, width=11)  
        food2.place(x=100, y=35)  
        profession2 = Entry(NewScreen11, width=11)  
        profession2.place(x=100, y=70)  
        adjective2 = Entry(NewScreen11, width=11)  
        adjective2.place(x=100, y=105)  
        phrase = Entry(NewScreen11, width=11)  
        phrase.place(x=100, y=140)  
        animal2 = Entry(NewScreen11, width=11)  
        animal2.place(x=100, y=175)  
        verb2 = Entry(NewScreen11, width=11)  
        verb2.place(x=100, y=210)  
        place2 = Entry(NewScreen11, width=11)  
        place2.place(x=100, y=245)  
        celebrity2 = Entry(NewScreen11, width=11)  
        celebrity2.place(x=100, y=280)  
        somethingToBuy = Entry(NewScreen11, width=11)  
        somethingToBuy.place(x=100, y=315)  
        things2 = Entry(NewScreen11, width=11)  
        things2.place(x=100, y=345)  
        SubmitButton1  = Button(NewScreen11, text="Submit", background="Brown", font=('Calibri', 12), command=lambda:final(NewScreen11, food2.get(), profession2.get(), adjective2.get(), phrase.get(), animal2.get(), verb2.get(), place2.get(), celebrity2.get(), somethingToBuy.get(), things2.get(),))  
        SubmitButton1.place(x=150, y=380)  
